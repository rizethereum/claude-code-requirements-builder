...
async function getCurrentRequirement(): Promise<string | null> {
  const currentFile = join(process.cwd(), 'requirements', '.current-requirement');
  return await readFileIfExists(currentFile);
}

async function setCurrentRequirement(folderName: string): Promise<void> {
  const requirementsDir = join(process.cwd(), 'requirements');
  await ensureDir(requirementsDir);
...
async function clearCurrentRequirement(): Promise<void> {
  const currentFile = join(process.cwd(), 'requirements', '.current-requirement');
  try {
...
function createTimestampFolder(name: string): string {
  const now = new Date();
...
server.registerTool(
  ...
  async ({ request }) => {
    try {
      ...
      await fs.writeFile(
        join(requirementPath, '00-initial-request.md'),
        initialRequestContent
      );
      
      await fs.writeFile(
        join(requirementPath, 'metadata.json'),
      ...
      await fs.writeFile(
        join(requirementPath, '01-discovery-questions.md'),
        questionsContent
      );
    ...
    try {
      ...
      if (metadata.phase === "discovery" && metadata.progress.discovery.answered < metadata.progress.discovery.total) {
        ...
        const answer = await askQuestion(
          server,
          `Discovery Question ${currentQuestionIndex + 1}/${metadata.progress.discovery.total}: ${currentQuestion.question}`,
      ...
      if (metadata.phase === "detail" && metadata.progress.detail.answered < metadata.progress.detail.total) {
        ...
        const answer = await askQuestion(
          server,
          `Expert Question ${currentQuestionIndex + 1}/${metadata.progress.detail.total}: ${currentQuestion.question}`,
          currentQuestion.defaultValue,
          currentQuestion.reason
        );
...
async function askQuestion(
  ...
  if (result.action === "accept" && result.content?.answer !== undefined) {
    return result.content.answer as boolean;
  }
...
function generateDiscoveryQuestions(count: number): Array<{question: string, defaultValue: boolean, reason: string}> {
  const allQuestions = [
    {
